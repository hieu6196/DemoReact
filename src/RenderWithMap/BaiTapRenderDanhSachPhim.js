...
const renderPhimWithMap = () => {
    const objectStyle = {
        width: 'auto',
        height: '200px'
    }
    const arrPhim = mangPhim.map((phim, i) => {
        return <div className="col-4 mb-2" key={i}>
            <div className="card text-left">
                <img style={{
                    width: 'auto',
                    height: '200px'
                }} className={`card-img-top`} src={phim.hinhAnh} alt />
                ...
                <div className="card-body">
                    ...
                    <p className="card-text" style={{ height: '100px' }}>{phim.moTa.length > 150 ? <span>{phim.moTa.substr(0, 150)}...</span> : <span>{phim.moTa}</span>}</p>
                </div>
            </div>
        </div >;
    })
    ...
    render() {
        return (
            <div className="container">
                <h1 className="text-danger text-center">Danh sách phim</h1>
                <div className="row">
                    {renderPhimWithMap()};
                </div>
            </div>
